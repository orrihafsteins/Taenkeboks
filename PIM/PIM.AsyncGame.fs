...
type AsyncGame<'S,'A,'V>(game:Game<'S,'A,'V>,playerNames:string[],ct:CancellationToken) =
    ...
    let visibleCurrent : 'V[] = Array.empty
    let players = 
        playerNames
        |> Array.mapi (fun i n ->
            {
    ...
    let updateAllPlayers state = 
        async {
            for player in players do
                ...
                do! visibleChannels.[player.Side].Writer.WriteAsync(visible,ct).AsTask() |> Async.AwaitTask 
        }
    ...
    let errorPlayer error side = 
        async {
            let player = players.[side]
            do! errorChannels.[player.Side].Writer.WriteAsync(error,ct).AsTask() |> Async.AwaitTask 
        }                
    member this.RunGame () =
        async {
